...
- **변경사항 미리보기 가독성** — 적용 전 미리보기에 탈퇴·신규·닉네임 변경 멤버의 **시트 행 번호**를 병기하고 추가(＋)/삭제(－)/변경(～)을 색상·기호로 구분 (중복 닉네임도 행으로 식별)
- **수집 진행 표시** — 데이터를 가져오는 동안 도구 화면에 수집 진행 상태 표시 (유저스크립트 v2.5.0+, 구버전은 표시만 생략)
...
4. **(선택) 유니온 멤버 탭 준비** — 가입 순서대로 32명을 미리 입력해도 되고, **비워둔 채 시작**하면 첫 실행 때 도구가 자동으로 채워줍니다 (자동 채움은 가입 순서가 아닌 수집 순서 → 이후 직접 정렬)
...
- **유니온 멤버 입력은 선택**입니다. 미리 가입 순서대로 작성해두거나, **완전히 빈 상태로 시작**하면 첫 실행 시 도구가 수집한 닉네임으로 자동으로 채워줍니다. 단 **자동 채움은 가입 순서가 아닌 수집 순서**이므로, 가입 순서가 중요하면 시트에서 직접 정렬해주세요. (닉네임이 이미 있는 시트는 자동 채움이 동작하지 않아 기존 데이터를 덮어쓰지 않습니다.)
